...
const loadUserData = useCallback(async () => {
  try {
    if (isDevModeEnabled()) {
      ...
      // **CRITICAL FIX**: Persist dev movies to AsyncStorage for enhanced rating system
      try {
        await Promise.all([
          storageService.setItem(STORAGE_KEYS.MOVIES.SEEN, devMovies),
          storageService.setItem(STORAGE_KEYS.TV_SHOWS.SEEN, devTVShows),
          storageService.setItem(STORAGE_KEYS.MOVIES.UNSEEN, []),
          storageService.setItem(STORAGE_KEYS.TV_SHOWS.UNSEEN, [])
        ]);
        console.log('🔧 DEV MODE: Persisted dev movies and TV shows to AsyncStorage');
      } catch (error) {
        console.error('🔧 DEV MODE: Failed to persist to AsyncStorage:', error);
      }
