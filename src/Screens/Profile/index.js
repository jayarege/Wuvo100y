...
const ProfileScreen = ({ seen = [], unseen = [], seenTVShows = [], unseenTVShows = [], isDarkMode, navigation, onUpdateRating, onAddToSeen, onRemoveFromWatchlist, genres, route }) => {
  ...
  const [activeProfileTab, setActiveProfileTab] = useState('toppicks'); // 'toppicks' or 'watchlist'
...
Please rate a few more movies first!`,
 ...
 const handleConfirmRating = useCallback((updated) => {
  const finalRating = updated?.userRating ?? updated?.finalRating;
  console.log('✅ PROFILE: Confirming rating:', finalRating, 'for:', selectedMovie?.title);

  if (!selectedMovie || !finalRating) return;

  // Keep for any UI that reads it
  setFinalCalculatedRating(finalRating);

  // Merge with any existing seen entry so we don’t lose BT fields
  const existing = (Array.isArray(seen) ? seen.find(m => m.id === selectedMovie.id) : null) || {};

  const movieToAdd = {
    // identity / title
    id: selectedMovie.id,
    title: selectedMovie.title || selectedMovie.name || existing.title,
    mediaType: selectedMovie.mediaType || existing.mediaType || mediaType,

    // artwork
    poster: selectedMovie.poster_path || selectedMovie.poster || existing.poster,
    poster_path: selectedMovie.poster_path ?? existing.poster_path,

    // metadata
    score: selectedMovie.vote_average ?? selectedMovie.score ?? existing.score ?? 0,
    vote_average: selectedMovie.vote_average ?? existing.vote_average,
    voteCount: selectedMovie.vote_count ?? existing.voteCount ?? 0,
    release_date: selectedMovie.release_date || selectedMovie.first_air_date || existing.release_date,
    genre_ids: selectedMovie.genre_ids || existing.genre_ids || [],
    overview: selectedMovie.overview ?? existing.overview ?? "",
    adult: selectedMovie.adult ?? existing.adult ?? false,

    // RATINGS — always prefer fresh values from `updated`
    userRating: finalRating,
    // carry over BT/confidence fields
    confidencePercent: updated?.confidencePercent ?? existing.confidencePercent ?? null,
    confidenceWidth: updated?.confidenceWidth ?? existing.confidenceWidth ?? null,
    comparisons: updated?.comparisons ?? existing.comparisons ?? 0,
    wins: updated?.wins ?? existing.wins ?? 0,
    losses: updated?.losses ?? existing.losses ?? 0,
    theta: updated?.theta ?? existing.theta ?? null,
    thetaPrior: updated?.thetaPrior ?? existing.thetaPrior ?? null,

    // user context
    ratingCategory: selectedCategory ?? existing.ratingCategory ?? null,
    sentiment: selectedEmotion ?? existing.sentiment ?? null,
  ...
  setComparisonModalVisible(false);
  setMovieDetailModalVisible(false); // close details after rating
  setSelectedMovie(null);
  setSelectedCategory(null);
  setComparisonMovies([]);
  setCurrentComparison(0);
...
}, [selectedMovie, selectedCategory, selectedEmotion, seen, mediaType, onAddToSeen]);
            ...
            <View style={[
              ...
              {showGenreDropdown && (
                <View>
                  ...
                  <View style={profileStyles.advancedFiltersContainer}>
                    <TouchableOpacity 
                      style={[profileStyles.advancedFiltersToggle, { backgroundColor: colors.accent + '20', borderColor: colors.accent }]}
                    ...
                    {advancedFilterVisible && (
                      <View style={profileStyles.advancedFilterContainer}>
                        ...
                        {(selectedAdvancedStreamingServices.length > 0 || 
                          ...
                          friendRatingMin !== null || friendRatingMax !== null) && (
                          <TouchableOpacity
                            style={[profileStyles.clearAdvancedFiltersButton, { borderColor: colors.accent }]}
                            onPress={() => {
                            ...
                            activeOpacity={0.7}
                          >
                            <Text style={[profileStyles.clearAdvancedFiltersButtonText, { color: colors.accent }]}>
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: 'transparent' }]}>
        ...
        <View style={[styles.tabContainer, { backgroundColor: colors.background }]}>
          <TouchableOpacity 
            style={[
              styles.tabButton, 
              activeProfileTab === 'toppicks' && { 
                borderBottomColor: colors.accent, 
                borderBottomWidth: 2 
              }
            ]}
            onPress={() => setActiveProfileTab('toppicks')}
          >
            <Text 
              style={[
                styles.tabButtonText,
                { 
                  color: activeProfileTab === 'toppicks' ? colors.accent : colors.subText,
                  fontWeight: activeProfileTab === 'toppicks' ? 'bold' : 'normal'
                }
              ]}
            >
          ...
          <TouchableOpacity 
            ...
            onPress={() => setActiveProfileTab('watchlist')}
          >
            <Text 
              style={[
                styles.tabButtonText,
                { 
                  color: activeProfileTab === 'watchlist' ? colors.accent : colors.subText,
                  fontWeight: activeProfileTab === 'watchlist' ? 'bold' : 'normal'
                }
              ]}
            >
              Watchlist
              ...
              {mediaFilteredMovies
                ...
                .map((movie, index) => (
                  ...
                  >
                    ...
                    <LinearGradient
                      colors={colors.primaryGradient}
                      style={styles.posterGradientWrapper}
                    >
                      <Image
                        source={{ uri: getPosterUrl(movie.poster || movie.poster_path) }}
                    ...
                    <View style={styles.ratingsContainer}>
                      <View style={styles.ratingColumn}>
                        <Text style={[styles.ratingLabel, { color: colors.subText }]}>USER</Text>
              ...
              {moviesByMediaType
                ...
                .map((movie, index) => (
                  <TouchableOpacity
                    key={movie.id}
                    style={[styles.listItem, { backgroundColor: colors.card }]}
                    onPress={() => handleMovieSelect(movie, 'watchlist')}
                    activeOpacity={0.7}
                  >
                    <LinearGradient
                      colors={colors.primaryGradient}
  ...
  handleConfirmRating(updatedMovie); // pass the whole object so confidence/comparisons/etc are preserved
  setComparisonModalVisible(false);
}}
